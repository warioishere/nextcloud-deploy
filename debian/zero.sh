...
echo ""
if ([ "$(lsb_release -r | awk '{ print $2 }')" = "12" ] ||  [ "$(lsb_release -r | awk '{ print $2 }')" = "13" ]);
	then
...
echo -e " »\e[1;31m"
echo "******************************************************"
echo "*   Sie verwenden KEIN DEBIAN 12 / 13 x86_64         *"
echo "******************************************************"
echo  -e "\033[0m"
echo ""
exit 1
...
if [ -e "/var/www/nextcloud/config/config.php" ] || [ -e /etc/nginx/conf.d/nextcloud.conf ]; then
  ...
  echo ""
  echo "* Bitte setzen Sie den Server ggf. neu auf, bevor Sie mit einer Installation fortfahren."
  echo "* Please re-setup the server again before proceeding to a new installation."
  echo ""
  exit 1
else
  echo "*************************************************"
...
if [ ! -d "/home/$BENUTZERNAME/Nextcloud-Installationsskript/" ]; then
  ...
  # if ([ "$(lsb_release -r | awk '{ print $2 }')" = "13" ]);
  # then
  # PHPVERSION="8.4"
  # fi
  seconds=2
...
${apt} update
${apt} upgrade -y
# "SOFTWARE-PROPERTIES-COMMON" removed by debian:
if ([ "$(lsb_release -r | awk '{ print $2 }')" = "12" ]);
then
${apt} install -y \
...
libfile-fcntllock-perl libfontconfig1 libfuse2 locate npm net-tools rsyslog screen smbclient socat software-properties-common \
ssl-cert tree unzip wget zip
else
${apt} install -y \
apt-transport-https bash-completion bzip2 ca-certificates cron curl dialog dirmngr ffmpeg ghostscript gpg gnupg gnupg2 htop jq \
libfile-fcntllock-perl libfontconfig1 libfuse2 locate npm net-tools rsyslog screen smbclient socat ssl-cert tree unzip wget zip
fi
...
#Types: deb
#URIs: https://dlm.mariadb.com/repo/mariadb-server/11.8/repo/debian
#Suites: bookworm
#Components: main
# Architectures: amd64
...
#WHICH_DEBIAN_VERSION="$(lsb_release -r | awk '{ print $2 }')"
#if [ "$WHICH_DEBIAN_VERSION" == "13" ]; then
...
#fi
#fi
#${echo} -e "Package: *\nPin: origin dlm.mariadb.com\nPin-Priority: 900\n" | sudo tee /etc/apt/preferences.d/99mariadb
echo "... Will install MariaDB from DEBIAN Repo..."
else
...
${apt} install -y libmagickcore-7.q16-10-extra --allow-change-held-packages 
fi
...
${sed} -i 's/memory_limit = 128M/memory_limit = 1G/' /etc/php/$PHPVERSION/fpm/php.ini
...
cd /
${apt} install -y php$PHPVERSION-pgsql postgresql-17 --allow-change-held-packages
sudo -u postgres psql -c "CREATE USER ${NCDBUSER} WITH PASSWORD '${NCDBPASSWORD}' CREATEDB;"
sudo -u postgres psql -c "CREATE DATABASE ${NCDBNAME} TEMPLATE template0 ENCODING 'UTF8';"
...
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON SCHEMA public TO ${NCDBUSER};"
${systemctl} restart postgresql
...
sudo -u postgres psql -c "ALTER SYSTEM SET work_mem = '20164kB';"
...
sudo -u postgres psql -c "ALTER SYSTEM SET max_worker_processes = '4';"
sudo -u postgres psql -c "ALTER SYSTEM SET max_parallel_workers_per_gather = '2';"
sudo -u postgres psql -c "ALTER SYSTEM SET max_parallel_workers = '4';"
sudo -u postgres psql -c "ALTER SYSTEM SET max_parallel_maintenance_workers = '2';"
else
# -- DB Version: 16
# -- OS Type: linux
# -- DB Type: web
...
sudo -u postgres psql -c "ALTER SYSTEM SET random_page_cost = '1.1';"
sudo -u postgres psql -c "ALTER SYSTEM SET effective_io_concurrency = '200';"
sudo -u postgres psql -c "ALTER SYSTEM SET work_mem = '9709kB';"
sudo -u postgres psql -c "ALTER SYSTEM SET huge_pages = 'off';"
sudo -u postgres psql -c "ALTER SYSTEM SET min_wal_size = '1GB';"
sudo -u postgres psql -c "ALTER SYSTEM SET max_wal_size = '4GB';"
sudo -u postgres psql -c "ALTER SYSTEM SET max_worker_processes = '4';"
...
${cat} <<EOF >>/var/www/nextcloud/config/config.php
  ...
  array (
    ...
    array (
      ...
      // 'maximum.supported.desktop.version' => 'x.y.u',
...
enabled = true
EOF
${touch} /var/log/nextcloud/$NEXTCLOUDDNS.log
...
if [ "$(lsb_release -r | awk '{ print $2 }')" = "13" ]
then
sed -i 's/trixie/bookworm/' /etc/apt/sources.list.d/crowdsec_crowdsec.list
fi
${apt} update
...
y
EOF
${systemctl} restart redis-server.service ufw.service
###########################
...
${echo} ""
sleep 1
${sudo} -u www-data /usr/bin/php /var/www/nextcloud/occ app:disable app_api
